...
import { OhdabPost } from '@app-types/ohdab';
...
export const MyScreen: React.FC = () => {
  ...
  const [selectedPostForCard, setSelectedPostForCard] =
    useState<OhdabPost | null>(null);
  ...
  const karmaPoints =
    userPosts.length * 100 + commentCount * 50 + voteCount * 20;
      ...
      >
          ...
          <View className="flex-row items-center">
            ...
            <View className="ml-4 flex-1">
              ...
              <View className="bg-brand-yellow/12 rounded-lg px-2 py-1 self-start mt-1.5">
                <Text className="text-tag-sub font-bold text-brand-yellow">
                  카르마 점수: {karmaPoints}P
                </Text>
              </View>
            </View>
          </View>
          ...
          <View className="flex-row justify-between items-center mb-3">
            ...
            {notifications.length > 0 && (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text className="text-tag-sub font-bold text-neutral-slate">
                  전체 삭제
                </Text>
              </TouchableOpacity>
            )}
          ...
          {notifications.length === 0 ? (
            <View className="py-4 items-center">
              <Text className="text-tag-sub text-neutral-slate text-center">
                도착한 알림이 없습니다. 사연을 작성하거나 투표하면 여기에 실시간
                피드백이 전송됩니다!
              </Text>
            </View>
          ) : (
            <View className="gap-2">
              {notifications.map(notif => (
                <View
                  key={notif.id}
                  className="flex-row items-center bg-[#F7F2FA] border border-neutral-outline/15 rounded-xl p-3"
                >
                  <View className="w-2 h-2 rounded-full bg-brand-rose mr-2.5" />
                  <Text className="flex-1 text-tag-sub text-neutral-charcoal">
                    {notif.message}
                  </Text>
                </View>
              ))}
        ...
        {userPosts.length === 0 ? (
          <View className="bg-white rounded-[24px] border border-neutral-outline/15 p-4 mb-5 shadow-sm items-center py-8">
            ...
            <Text className="text-body-text text-neutral-slate mt-2">
              아직 작성된 오답노트가 없습니다.
            </Text>
          </View>
        ) : (
          userPosts.map(post => (
            ...
            >
              <View className="flex-1 pr-3">
                <Text
                  className="text-body-text font-bold text-neutral-charcoal"
                  numberOfLines={1}
                >
                  {post.hook}
                </Text>
              ...
              <View className="bg-brand-rose/12 rounded-lg px-2.5 py-1.5">
                <Text className="text-tag-sub font-bold text-brand-rose">
                  채점 카드
                </Text>
              </View>
            </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text className="text-body-text font-bold text-[#E53935]">
            로그아웃
          </Text>
        </TouchableOpacity>
      </ScrollView>
...
const BadgeItem: React.FC<{ badge: BadgeData }> = ({ badge }) => {
  ...
  const iconBgClasses = badge.isUnlocked
    ? 'bg-brand-yellow/15'
    : 'bg-transparent';
  const iconOpacityClasses = badge.isUnlocked ? 'opacity-100' : 'opacity-40';
  ...
  return (
    <View
      className={`w-24 h-[114px] rounded-xl border p-2 items-center justify-center mr-2.5 ${cardClasses}`}
    >
      <View
        className={`w-10 h-10 rounded-full justify-center items-center ${iconBgClasses} ${iconOpacityClasses}`}
      >
        <IconComponent
          color={badge.iconColor}
          size={20}
          fill={badge.isUnlocked ? badge.iconColor : 'none'}
        />
      </View>
      <Text
        className={`text-tag-sub font-bold text-center mt-1.5 ${nameColorClasses}`}
      >
        {badge.name}
      </Text>
      <Text
        className="text-tag-sub text-neutral-slate text-center mt-0.5"
        numberOfLines={2}
      >
        {badge.desc}
      </Text>
...
const GradingCardDialog: React.FC<DialogProps> = ({ post, onDismiss }) => {
  ...
  const partnerFaultPercent =
    totalVotes > 0 ? Math.round((post.votePartnerFault * 100) / totalVotes) : 0;
  const authorFaultPercent =
    totalVotes > 0 ? Math.round((post.voteAuthorFault * 100) / totalVotes) : 0;
    ...
    <Modal transparent={true} animationType="fade">
      <View className="flex-1 bg-black/50 justify-center items-center p-5">
        <View className="w-full max-h-[90%] bg-bg-paper rounded-2xl border-2 border-red-500 p-5 relative overflow-hidden">
          ...
          <Text
            className="absolute right-12 top-[60px] text-hero-hook scale-[8.0] font-bold text-red-500/5 z-0"
            pointerEvents="none"
          >
            ✓
          </Text>
          ...
          >
            ...
            <View className="flex-row items-center justify-center gap-1.5">
              ...
              <Text className="text-logo-title font-bold text-slate-800 text-center">
                연애 오답 시험 채점표
              </Text>
            </View>
            <Text className="text-tag-sub text-slate-600 text-center mt-1">
              제출자 닉네임: {post.nickname}
            </Text>
            ...
            <View className="flex-row justify-between items-center mb-5">
              <View className="flex-1 pr-2.5">
                <View className="flex-row items-center gap-1 mb-1">
                  ...
                  <Text className="text-tag-sub font-bold text-red-500">
                    핵심 흑역사 훅:
                  </Text>
                </View>
                <Text className="text-logo-title font-extrabold text-slate-800 leading-[18px]">
                  "{post.hook}"
                </Text>
              </View>
              ...
              <View className="w-20 h-20 rounded-full border-[3px] border-red-500 justify-center items-center">
                <Text className="text-hero-hook font-black text-red-500">
                  {getVerdictGrade()}
                </Text>
                <Text className="text-tag-sub font-bold text-red-500 mt-0.5 scale-[0.8]">
                  오답 판정
                </Text>
              </View>
            </View>
            ...
            <View className="bg-slate-100 rounded-xl border border-slate-200 p-3.5 mb-6">
              <Text className="text-widget-title font-bold text-slate-700 mb-2">
                최종 대중 과실 비율
              </Text>
              <View className="flex-row justify-between mb-2.5">
                <Text className="text-body-text font-black text-red-500">
                  상대방 과실: {partnerFaultPercent}%
                </Text>
                <Text className="text-body-text font-semibold text-slate-500">
                  나의 과실: {authorFaultPercent}%
                </Text>
              </View>
              <Text className="text-tag-sub text-slate-600">
                {getVerdictText()}
              </Text>
            </View>
            ...
            {/* Share action */}
            <TouchableOpacity
              className="bg-[#E1306C] rounded-xl py-3.5 items-center mb-2"
              activeOpacity={0.8}
            >
              <Text className="text-body-text font-bold text-white">
                인스타그램 스토리 공유하기
              </Text>
            </TouchableOpacity>
            ...
            {/* Close Button */}
            <TouchableOpacity
              className="border border-slate-500 rounded-xl py-3 items-center"
              onPress={onDismiss}
              activeOpacity={0.8}
            >
              <Text className="text-body-text font-bold text-slate-700">
                창 닫기
              </Text>
            </TouchableOpacity>
          </ScrollView>
