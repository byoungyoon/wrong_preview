...
export const FeedScreen: React.FC = () => {
  ...
  const getRankingScore = (post: OhdabPost) => {
    return (
      (post.voteAuthorFault + post.votePartnerFault) * 12 +
      post.sympathyCount * 7 +
      post.angerCount * 8
    );
  };
    ...
    <SafeAreaView className="flex-1 bg-bg-lavender">
      ...
      <View className="h-[60px] bg-white/75 border-b border-neutral-outline/30 flex-row justify-between items-center px-5 shadow-sm">
        <View className="flex-row items-center gap-1">
          <Text className="text-logo-title font-black text-brand-purple">
            오답연애
          </Text>
        ...
        <View className="flex-row bg-neutral-outline/20 rounded-[30px] p-[3px]">
          ...
          >
            <Text
              className={`text-tag-sub font-bold ${
                activeTab === 'home'
                  ? 'text-brand-purple'
                  : 'text-neutral-slate'
              }`}
      ...
      >
        {displayPosts.length === 0 ? (
          <View className="flex-1 justify-center items-center py-24">
            <Text className="text-body-text text-neutral-slate">
              등록된 오답노트가 없습니다.
            </Text>
          </View>
        ) : (
...
const OhdabPostCard: React.FC<PostCardProps> = ({
  ...
  const getAvatar = () => {
    switch (post.avatarId) {
      case 'avatar_crayon_red':
        return <Text className="text-logo-title">😡</Text>;
      case 'avatar_crayon_yellow':
        return <Text className="text-logo-title">🤔</Text>;
      case 'avatar_crayon_blue':
        return <Text className="text-logo-title">😭</Text>;
      default:
        return <User color="#21005D" size={16} />;
    }
  };
  ...
  const renderRankBadge = () => {
    ...
    return (
      <View
        className={`rounded-2xl px-4 py-2 border-[1.5px] flex-row justify-between items-center my-1 shadow-sm ${badgeBgBorder}`}
      >
        <View className="flex-row items-center gap-1.5">
          <Trophy
            color={iconColor}
            size={16}
            fill={rank <= 3 ? iconColor : 'none'}
          />
        ...
        {score !== undefined && (
          <Text className="text-tag-sub font-black text-neutral-slate">
            공감지수 {score}점
          </Text>
        )}
      </View>
  ...
  return (
    ...
    >
      ...
      <View className="items-center justify-center min-h-[180px] relative my-2.5">
        <Text className="text-hero-hook scale-[2.2] font-bold text-brand-rose self-center -mb-4">
          “
        </Text>
        <Text className="text-hero-hook font-extrabold text-[#1D1B20] text-center px-3">
          {post.hook}
        </Text>
          ...
          <View className="w-full mt-4">
            <NotebookPaper>
              <View className="w-full">
                ...
                <Text className="text-body-text text-neutral-charcoal/90">
                  {post.stepIntro}
                </Text>
              </View>
      ...
      <View className="gap-3 mt-2.5">
        ...
        <View className="flex-row gap-3">
          ...
          <View className="rounded-[20px] p-3 border-[1.5px] border-white/50 bg-white/70 shadow-sm flex-[1.2] flex-row items-center">
            ...
            <View className="ml-2 flex-1">
              <Text
                className="text-widget-title font-bold text-neutral-charcoal"
                numberOfLines={1}
              >
                {post.nickname}
              </Text>
              {post.isUserPost && (
                <View className="bg-brand-rose/20 rounded px-1 py-0.5 mt-0.5 self-start">
                  <Text className="text-tag-sub font-bold text-brand-rose scale-[0.8] origin-left">
                    작성자
                  </Text>
                </View>
              )}
              ...
              <View className="flex-row items-center gap-1.5">
                {post.tags.split(',').map((tag, idx) => {
                  ...
                  return (
                    <View
                      key={idx}
                      className="bg-white rounded-lg px-2 py-1.5 border border-neutral-outline/20"
                    >
        ...
        <View className="rounded-[20px] p-3 border-[1.5px] border-white/50 bg-white/70 shadow-sm p-4">
          <Text className="text-widget-title font-bold text-neutral-charcoal mb-2.5">
            📊 이 상황, 누구 과실이 더 큰가요?
          </Text>
            ...
            <View className="flex-1 h-full relative">
              ...
              >
                ...
                >
                  ...
                  <Text className="text-tag-sub text-neutral-slate mt-0.5">
                    자책/후회
                  </Text>
                </TouchableOpacity>
              </Animated.View>
              ...
              <Animated.View
                ...
                className="w-full h-full absolute backface-hidden top-0 left-0 right-0 bottom-0"
              >
                <View className="w-full h-full rounded-2xl bg-bg-container border-[1.5px] border-neutral-outline items-center justify-center">
                  <Text
                    className={`text-tag-sub text-neutral-slate scale-[0.9] ${
                      post.userVoted === 1 ? 'font-bold' : ''
                    }`}
                  >
                    내가 오답{post.userVoted === 1 ? ' (내 투표)' : ''}
                  </Text>
                  <Text className="text-hero-hook font-black text-neutral-charcoal">
                    {authorPercent}%
                  </Text>
                  <Text className="text-tag-sub text-neutral-slate scale-[0.8]">
                    {post.voteAuthorFault}표
                  </Text>
                </View>
              </Animated.View>
            </View>
            ...
            <View className="flex-1 h-full relative">
              ...
              >
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text className="text-body-text font-bold text-brand-rose">
                    상대 오답
                  </Text>
                  <Text className="text-tag-sub text-brand-rose mt-0.5">
                    상대방 과실 100%
                  </Text>
                </TouchableOpacity>
              </Animated.View>
              ...
              >
                <View className="w-full h-full rounded-2xl bg-[#FFD8E4] border-[1.5px] border-brand-rose/40 items-center justify-center">
                  <Text
                    className={`text-tag-sub text-brand-rose scale-[0.9] ${
                      post.userVoted === 2 ? 'font-bold' : ''
                    }`}
                  >
                    상대 오답{post.userVoted === 2 ? ' (내 투표)' : ''}
                  </Text>
                  <Text className="text-hero-hook font-black text-brand-rose">
                    {partnerPercent}%
                  </Text>
                  <Text className="text-tag-sub text-brand-rose scale-[0.8]">
                    {post.votePartnerFault}표
                  </Text>
                </View>
              </Animated.View>
            </View>
...
const CommentItem: React.FC<{ comment: Comment }> = ({ comment }) => {
  ...
  const itemBgClass = isCoach
    ? 'bg-brand-rose/12 border-brand-rose/30'
    : 'bg-[#F7F2FA] border-neutral-outline/20';
  ...
  return (
    <View className={`rounded-2xl border p-3 mb-3 ${itemBgClass}`}>
      ...
      <Text className="text-body-text text-neutral-charcoal">
        {comment.text}
      </Text>
    </View>
  );
